import { getHiringKanban, getHubSpotDeals, HIRING_STAGES, type KanbanColumn } from '@/lib/hubspot';

// Stage colour theming — shared with the root portal
...
function getStage(label: string) {
    return STAGE_CONFIG[label] ?? { dot: 'bg-gray-400', bg: 'bg-gray-900/40', border: 'border-gray-700', text: 'text-gray-400' };
}
...
export default async function ClientPortalBySlugPage({ params }: PageProps) {
    const { clientSlug } = await params;

    // Fetch all deals and filter by matching client slug
    const allDeals = await getHubSpotDeals();
    const clientDeals = allDeals.filter(d => d.clientSlug === clientSlug);

    // Derive display name from first matching deal
    const clientDisplayName = clientDeals[0]?.client || decodeURIComponent(clientSlug)
        .split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');

    // Derive unique active roles for this client
    const activeRoles = [...new Set(clientDeals.map(d => d.role).filter(Boolean))];

    // Build Kanban columns — only the 7 hiring stages, filtered to this client
    const columns: KanbanColumn[] = Object.entries(HIRING_STAGES).map(([id, label]) => {
        const dealsInStage = clientDeals.filter(d => d.dealstage === id);
        return { stageId: id, label, count: dealsInStage.length, deals: dealsInStage };
    });
        ...
        <div className="px-6 py-10 max-w-screen-2xl mx-auto">
            ...
            <div className="mb-2">
                <p className="text-white/30 text-xs uppercase tracking-widest font-medium mb-3">
                    Exclusive Search · {clientDisplayName}
                </p>
            ...
            {activeRoles.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-8 mt-5">
                    {activeRoles.map(role => (
                        <span
                            key={role}
                            className="px-3 py-1 rounded-full border border-[var(--color-brand-orange)]/30 text-[var(--color-brand-orange)] text-xs tracking-wider uppercase font-medium bg-orange-950/20"
                        >
                            {role}
                        </span>
                    ))}
                                ...
                                <div className="flex flex-col gap-2 p-3 flex-grow">
                                    ...
                                    {col.deals.slice(0, 8).map(deal => (
                                        ...
                                        >
                                            ...
                                            {deal.role && (
                                                <p className="text-xs text-white/30 mt-1 truncate">{deal.role}</p>
                                            )}
                                        </div>
                                    ))}
                                    {col.deals.length > 8 && (
            ...
            <div className="mt-10 rounded-xl border border-white/10 bg-white/5 px-6 py-4 flex items-start gap-3">
                ...
                <p className="text-xs text-white/30 font-light leading-relaxed">
                    Candidate names are partially anonymised for confidentiality. Full profiles are available upon request and subject to mutual NDA. This dashboard reflects the live state of your L'Agence search engagement for <strong className="text-white/40">{clientDisplayName}</strong>.
                </p>
            </div>
        </div>
    );
