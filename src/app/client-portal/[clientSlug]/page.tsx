import { getHubSpotDeals, HIRING_STAGES, type KanbanColumn } from '@/lib/hubspot';

// Stage config — brand palette (matches global portal page)
    ...
    return (
        <div className="bg-[var(--color-brand-light)] min-h-screen">
            <div className="px-8 py-12 max-w-screen-2xl mx-auto">
                ...
                {/* Active roles — styled as brand pills */}
                {activeRoles.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-12">
                        {activeRoles.map(role => (
                            <span
                                key={role}
                                className="px-4 py-1.5 rounded-full border border-[var(--color-brand-orange)]/40 text-[var(--color-brand-orange)] text-xs tracking-wider uppercase font-semibold bg-[var(--color-brand-orange)]/5"
                            >
                                {role}
                            </span>
                        ))}
                ...
                <div className="grid grid-cols-3 gap-6 mb-14">
                    {[
                        ...
                        <div
                            key={stat.label}
                            className="rounded-2xl bg-white border border-gray-100 shadow-sm px-8 py-7 flex items-start justify-between hover:shadow-md transition-shadow"
                        >
                            <div>
                        ...
                        {columns.map(col => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col gap-2 p-3 flex-grow min-h-[180px]">
                                        {col.deals.length === 0 && (
                                        ...
                                        {col.deals.slice(0, 8).map(deal => (
                                            ...
                                            >
                                                ...
                                                {deal.role && (
                                                    <p className="text-xs text-gray-400 mt-1 truncate font-light">{deal.role}</p>
                                                )}
                                            </div>
                                        ))}
                                        {col.deals.length > 8 && (
                ...
                <div className="mt-12 border-t border-gray-200 pt-8 flex items-start gap-3">
                    ...
                    <p className="text-xs text-gray-400 font-light leading-relaxed max-w-2xl">
                        Candidate names are partially anonymised for confidentiality. Full profiles are available upon request and subject to mutual NDA. This dashboard reflects the live state of your L&apos;Agence search engagement for <strong className="text-[var(--color-brand-dark)] font-medium">{clientDisplayName}</strong>.
                    </p>
                </div>
            </div>
        </div>
